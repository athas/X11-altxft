...
-- and display.  You are required to manually free the 'XftMgr' via
-- 'freeXftMgr' when you are done with it.  As Xlib is not re-entrant,
-- a synchronisation mechanism must be used, so the 'XftMgr' includes
-- actions for obtaining and releasing atomic access to the display
-- via two 'IO' actions.  These will be executed before and after
-- objects allocated via the manager are released.  It is recommended
-- to use an 'Control.Concurrent.MVar' to implement a mutex for
-- synchronising the access, but if you are absolutely certain that
-- there will not be any concurrent attempts to access the display,
-- the actions can merely be @return ()@.
